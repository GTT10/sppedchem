...
c     **                     SpeedCHEM FORTRAN                       **
c     **                                                             **
c     **           Mechanism and thermodynamic data input            **
c     **                                                             **
c     **   Input files required: SpeedCHEM.dat                       **
c     **                         SCthermo.dat                        **
c     **                                                             **
...
c     Current Working precision
      use working_precision

c     Module containing mechanism data
...
c     Open file for data output
ck2015      open(unit=820,file='SpeedCHEM.out',status='unknown')
      open(unit=820,file=trim(mechdir)//"SpeedCHEM.out",status='unknown')

c     Check presence of mechanism input file
ck2015      nomefile = 'SpeedCHEM.dat'
      nomefile = trim(mechdir)//"SpeedCHEM.dat"
      inquire(file=nomefile,exist=present)

      if (present) then
ck2015         open(unit = 800,file='SpeedCHEM.dat')
         open(unit = 800,file=trim(mechdir)//"SpeedCHEM.dat")

c           Reading mechanism label
            read(800,"(A80)")mechlabel
            mechanism = trim(adjustl(mechlabel))

c           Reading number of species and reactions
            read(800,*)dummy,nr
            read(800,*)dummy,ns

c           Allocate species and reaction indices
            allocate(species(ns), reactions(nr))
            species   = [(j,j=1,ns)]
            reactions = [(j,j=1,nr)]

c           Number of problem equations
            neq = ns + 1
            uneq = 1.e0_dp / neq

c           Reading element number
            read(800,*)
            read(800,*)dummy,nel

c           Allocating variables dimensions
            call SCallocate
c            third_body(:,:) = 0.e0_dp
            call allocate(nr,ns,0,third_body_sp)


c           Reading elements names
            do j=1,nel
               read(800,*)elementi(j)
            end do

c           Reading species names and molecular weights
            read(800,*)
            do j=1,ns
              read(800,"(A18,E17.10)")specie(j),SCMW(j)
c              write(*,"(A18,E17.10)")specie(j),SCMW(j)
            end do
            uMW = 1.e0_dp/SCMW
C           Reading reaction data
            read(800,*)
            read(800,*)
c           Reactants stoichiometric coefficients
            read(800,*)np
c            stoich_r(:,:) = 0.0d0
            do j=1,np
              read(800,*)irow,icol,tmp
              call add_value(stoich_r_sp,irow,icol,tmp)
c              stoich_r(irow,icol) = tmp
            end do

c           Fix sparse matrix representation for unactive species
            stoich_r_sp%nc = ns

c           Maximum number of reactants per reaction
            allocate(count_reacs(nr))
            call sparse_internal_count(stoich_r_sp,count_reacs,dim=2)
            max_nreacs = maxval(count_reacs)
c            max_nreacs = maxval(count(stoich_r/=0.e0_dp, dim=2))


c           Products stoichiometric coefficients
            read(800,*)
            read(800,*)np
c            stoich_p(:,:) = 0.0d0
            do j=1,np
              read(800,*)irow,icol,tmp
c              stoich_p(irow,icol) = tmp
...
c           Maximum number of products per reaction
            call sparse_internal_count(stoich_p_sp,count_reacs,dim=2)
            max_nprods = maxval(count_reacs)
            deallocate(count_reacs)

c           Reading reactions collision frequencies
            read(800,*)
            do j=1,nr
              read(800,*)A0(j),Ainf(j),Arev(j)
            end do

c           Reading reactions activation energies
            read(800,*)
            do j=1,nr
              read(800,*)E0(j),Einf(j),Erev(j)
            end do

c           Reading reactions temperature exponents
            read(800,*)
            do j=1,nr
              read(800,*)b0(j),binf(j),brev(j)
            end do

c           Reading HIGH pressure limits explicitly
            read(800,*)
            read(800,*)dummy,np
            allocate(ire(nr))
            HIGH(1:nr) = .FALSE.
...
c           Reading LOW pressure limits explicitly
            read(800,*)
            read(800,*)dummy,np
            allocate(ire(nr))
            LOW(1:nr) = .FALSE.
...
c           Check which reactions are reversible
            read(800,*)
            read(800,*)dummy,np
...
c           Read third-body efficiencies
            read(800,*)
            read(800,*)dummy,np
c            third_body(:,:) = 0.e0_dp
            if (np.gt.0) then
            do j=1,np
              read(800,*)irow,icol,tmp
c              third_body(irow,icol) = tmp
              call add_value(third_body_sp,irow,icol,tmp)
            end do
            endif

c           Manipulate third_body efficiency matrices for speed
c            third_bodyT = transpose(third_body)

c           If coefficients are diminished by one, the reactor config
c           can have a sparse Jacobian since total concentration is
c           constant
c            third_body_beta = 0.e0_dp
            call allocate(nTHREE, ns, 0, tb_beta_sp   )
            do j=1,size(iTHREE)
               i = iTHREE(j)
c               where(third_body(i,:)/=1.e0_dp)
c     &           third_body_beta(:,i) = 1.e0_dp - third_body(i,:)
...
c            third_body_betaT = transpose(third_body_beta)
c           Fix number of columns
            tb_beta_sp%nc = ns

c           Initi alising nudiff (vectorized operation)
c           and completing variables' definition
c            nudiff = stoich_p - stoich_r

            REV = ((Arev.ne.0.0d0).or.(brev.ne.0.0d0).or.
     &             (Erev.ne.0.0d0))

c           ** REVERSE REACTION RATES - related indices ****************

c           Index for simple, third-body reactions
c           (Effective molecularity but not Troe of Lindemann forms)
...
c            if (.not.allocated(isumnudiff)) then
c               allocate(isumnudiff(nr))
c               isumnudiff = int(sum(nudiff,2))
c            endif
...
c     *****************************************************************
c     Importing SpeedCHEM thermodynamic database

ck2015      nomefile = 'SCthermo.dat'
...
c       reading records in the thermodynamic database and associating
c       them to the species
...
c       Storing element data
        do j=1,ns
          read(810,940)nels(j),dummy1,ne1,dummy2,ne2,
     &                         dummy3,ne3,dummy4,ne4
c          write(*,940)nels(j),dummy1,ne1,dummy2,ne2,
c     &                         dummy3,ne3,dummy4,ne4
...
c     Allocating variables for TROE pressure-dependent reactions
...
c     ******************************************************************
c     Allocating types of reactions in module reacpar
...
c     PRODUCTS ********************************************************
c     (data is only needed for reversible reactions)


c     jp, ip = row and column indexes (in stoich_p matrix) referring
c     to non-zero stoichiometric coefficients of species active in
...
c      do i=1,size(i1r,1)
c      write(*,"(5(I5))")i,indice_r(i,1),indice_r(i,2),i1r(i,1),i1r(i,2)
c      end do
c      pause
...
c     Preparing output file containing mechanism data
...
c     ** Writing TSS-solver-related information ***********************

c      write(820,*)'TSS solver-related information*************************
c     &************************************************'

c         write(820,951)'isolver',isolver
c         write(820,951)'ifast',ifast
c         write(820,950)'dt_def',dt_def
c         write(820,950)'threshold',qfqb_threshold
c         write(820,950)'TSSfac',MTSfac
c         write(820,950)'TSSTOL',TSSTOL
c         write(820,950)'RTOL',TOLR
c         write(820,950)'ATOL',TOLA
c         write(820,950)
c         write(820,950)'thugeTS',thugeTS
c         write(820,951)'TSSinj1',TSSinj1
c         write(820,951)'TSSinj2',TSSinj2
...
c     **                     SpeedCHEM FORTRAN                       **
c     **                                                             **
c     **          Build numerical dense Jacobian matrix              **
c     **                                                             **
c     **                                                             **
c     **   Author:      (C) Federico Perini                          **
...
c     Evaluate derivative at point
      call SC_conV(neq,tempo,yin,dydt0)

      y = yin

c     Evaluate finite differences by small perturbations
      do i=1, neq
...
c     **                     SpeedCHEM FORTRAN                       **
c     **                                                             **
c     **            Evaluate jacobian sparsity pattern               **
c     **                                                             **
c     **                                                             **
c     **   Author:      (C) Federico Perini                          **
c     **   Date created: sunday, 23/10/2011                          **
c     **   Last update : friday, 17/08/2012                          **
c     **                                                             **
c     **   Usage, copying, distribution of the proprietary routines  **
c     **   or any modified versions of them are not allowed without  **
c     **   approval of the author.                                   **
...
c     **                     SpeedCHEM FORTRAN                       **
c     **                                                             **
c     **              DUMMY SUBROUTINE for ODE solvers               **
c     **                                                             **
c     **                                                             **
