...
C Compute new ordering of rows/columns of Jacobian. --------------------
 ...
 170    IWK(IBR+I) = I
      NSP = LIWK + 1 - IPISP
C     FEDERICO PERINI: CHANGE FOR SPEED! Compute ordering only once
      IF (allocated(species_permutations)) THEN
       IWK(IPR:IPR+N-1)   = species_permutations(1:N)
       IWK(IPIC:IPIC+N-1) = species_inverse_permutations(1:N)
       IYS = 0
      ELSE
...
c       f - real variable
c       v - supplies a value to the driverf
...
c ** error.. insufficient storage
 110  flag = 10*n + 1
!      write(*,*)'insufficient storage ',umax
      return
...
c  ******  if l is not required, then comment out the following line  **
          ...
          do 8 j=jmin,jmax
            jumu = ju(mu+j)
            lkiu = lki * u(j)
   8        row(jumu) = row(jumu) + lkiu
c   8        row(ju(mu+j)) = row(ju(mu+j)) + lki * u(j)
   9      i = jrl(i)
          if (i .ne. 0) go to 7
c
