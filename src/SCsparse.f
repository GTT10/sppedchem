...
c          Calculate number of block columns of A (not AT),
c          based on CACHELINE:
           bcol = cacheline
           blkcols = AT%nr/bcol ! division among integers!
           if ( blkcols*bcol /= AT%nr ) blkcols = blkcols + 1

c          Loop through blkrows block columns:
...
c             Now, loop through the only column rhscols block of c & b:
c             Loop through the brow columns in this block:
...
c             Count the number of nonzeros in this block of rows:
c             Now, loop through the only column rhscols block of c & b:
