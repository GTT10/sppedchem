c **********************************************************************
c     Version modified by Federico Perini to accomplish for sparse
c     Jacobian matrix provided in analytical form (MF = 26)
c     for stiff systems of ODEs.
c     Federico Perini, 03/06/2012
c **********************************************************************
...
C
      EXTERNAL DVNLSD
      LOGICAL IHIT
      DOUBLE PRECISION ATOLI, BIG, EWTI, H0, HMAX, HMX, 
     1   RH, RTOLI, SIZE, TCRIT, TNEXT, TOLSF, TP
      INTEGER I, IER, IFLAG, IMXER, JCO, KGO, LENIW, LENJ, LENP, LENRW,
     1   LENWM, LF0, MBAND, MFA, ML, MU, NITER, 
     2   NSLAST
      CHARACTER*80 MSG
C
...
c      DIMENSION MORD(2)
      ...
      INTEGER NID,OMP_GET_THREAD_NUM      
C-----------------------------------------------------------------------
...
!      write(*,*)'DVODE called, thread. no = ',NID+1,Y(1),T
!      write(*,*)MORD(1),MXSTP0,FOUR,PT2,HUN
...
c **** [fp] now option 26 (stiff problem, analytical sparse JAC) is
c      allowed
      IF (MITER .LT. 0 .OR. MITER .GT. 6) GO TO 608
      IF (METH == 1 .AND. MITER == 6) GO TO 608
...
C-----------------------------------------------------------------------
 60   LYH = 21
      ...
      IF (MITER .EQ. 1 .OR. MITER .EQ. 2 .OR. MITER == 6) THEN
        LENWM = 2 + (1 + JCO)*N*N
        LOCJS = N*N + 3
      ENDIF
...
C Type declarations for local variables --------------------------------
C
      DOUBLE PRECISION AFI, ATOLI, DELYI, H, HG, HLB, HNEW, HRAT,
     1     HUB, T1, TDIST, TROUND, YDDNRM
      INTEGER I, ITER
C
C Type declaration for function subroutines called ---------------------
C
...
c      SAVE HALF, HUN, PT1, TWO
      ...
      double precision, parameter :: half = 0.5d0, hun = 100.d0,
     &                               pt1  = 0.1d0, two = 2.d0
c      DATA HALF /0.5D0/, HUN /100.0D0/, PT1 /0.1D0/, TWO /2.0D0/
...
C Type declarations for local variables --------------------------------
C
c      DOUBLE PRECISION C, HUN, R, S, TFUZZ, TN1, TP, ZERO
      DOUBLE PRECISION C, R, S, TFUZZ, TN1, TP
      INTEGER I, IC, J, JB, JB2, JJ, JJ1, JP1
      CHARACTER*80 MSG
      DOUBLE PRECISION, PARAMETER :: HUN = 100.0D0, ZERO = 0.0D0
C-----------------------------------------------------------------------
C The following Fortran-77 declaration is to cause the values of the
C listed (local) variables to be saved between calls to this integrator.
C-----------------------------------------------------------------------
...
C Type declarations for local variables --------------------------------
C
c      DOUBLE PRECISION ADDON, BIAS1,BIAS2,BIAS3, CNQUOT, DDN, DSM, DUP,
c     1     ETACF, ETAMIN, ETAMX1, ETAMX2, ETAMX3, ETAMXF,
c     2     ETAQ, ETAQM1, ETAQP1, FLOTL, ONE, ONEPSM,
c     3     R, THRESH, TOLD, ZERO
      DOUBLE PRECISION CNQUOT, DDN, DSM, DUP, ETAQP1,FLOTL,R
...
c     1     ETACF, ETAMIN, ETAMX1, ETAMX2, ETAMX3, ETAMXF, ETAQ, ETAQM1,
...
C
      INTEGER, PARAMETER :: KFC = -3, KFH = -7, MXNCF = 10
      DOUBLE PRECISION, PARAMETER :: ONE = 1.D0, ZERO = 0.D0
      DOUBLE PRECISION, PARAMETER ::
     &     ADDON = 1.0D-6 ,    BIAS1 = 6.0D0 ,     BIAS2 = 6.0D0 ,
...
C      DATA ADDON  /1.0D-6/,    BIAS1  /6.0D0/,     BIAS2  /6.0D0/,
C     1     BIAS3  /10.0D0/,    ETACF  /0.25D0/,    ETAMIN /0.1D0/,
C     2     ETAMXF /0.2D0/,     ETAMX1 /1.0D4/,     ETAMX2 /10.0D0/,
C     3     ETAMX3 /10.0D0/,    ONEPSM /1.00001D0/, THRESH /1.5D0/
C      DATA ONE/1.0D0/, ZERO/0.0D0/
C
      KFLAG = 0
      TOLD = TN
      NCF = 0
...
C Type declarations for local variables --------------------------------
C
      DOUBLE PRECISION AHATN0, ALPH0, CNQM1, CSUM, ELP, EM,
     1     EM0, FLOTI, FLOTL, FLOTNQ, HSUM, RXI, RXIS, S, 
     2     T1, T2, T3, T4, T5, T6, XI
...
C
      double precision, parameter :: cortes = 0.1d0, one = 1.d0, 
     &                               two = 2.d0, six = 6.d0, zero = 0.d0
c      DATA CORTES /0.1D0/
c      DATA ONE  /1.0D0/, SIX /6.0D0/, TWO /2.0D0/, ZERO /0.0D0/
C
      FLOTL = REAL(L)
      NQM1 = NQ - 1
      NQM2 = NQ - 2
...
C Type declarations for local variables --------------------------------
C
      DOUBLE PRECISION ALPH0, ALPH1, HSUM, PROD, T1, XI,XIOLD
...
c        YH(1:n, j) = EL(j)*YH(1:n, LP1) + YH(1:n, j)
 370  CONTINUE
...
C Type declarations for local variables --------------------------------
C
      DOUBLE PRECISION CSCALE, DCON, DEL, DELP !ONE,
c     1     RDIV, TWO, ZERO
...
C-----------------------------------------------------------------------
      ...
      double precision, parameter :: one = 1.d0, zero = 0d0, two = 2.d0,
     &                               ccmax = 0.3d0, crdown = 0.3d0,
     &                               rdiv = 2.d0
      integer, parameter :: maxcor = 3, msbp = 20
...
c      DATA CCMAX /0.3D0/, CRDOWN /0.3D0/, MAXCOR /3/, MSBP /20/,
c     1     RDIV  /2.0D0/
c      DATA ONE /1.0D0/, TWO /2.0D0/, ZERO /0.0D0/
...
C Type declarations for local variables --------------------------------
C
      DOUBLE PRECISION CON, DI, FAC, HRL1, R, R0, SRUR, YI, YJ, YJJ
...
c      SAVE ONE, PT1, THOU, ZERO
      double precision, parameter :: one = 1.d0, thou = 1.d3, 
     &                               zero = 0.d0, pt1 = 1.d-1
C-----------------------------------------------------------------------
      COMMON /DVOD01/ ACNRM, CCMXJ, CONP, CRATE, DRC, EL(13),
     1                ETA, ETAMAX, H, HMIN, HMXI, HNEW, HSCAL, PRL1,
...
C
c      DATA ONE /1.0D0/, THOU /1000.0D0/, ZERO /0.0D0/, PT1 /0.1D0/
C
      IERPJ = 0
      HRL1 = H*RL1
...
C If JOK = -1 and MITER = 1, call JAC to evaluate Jacobian. ------------
 ...
 110    WM(I+2) = ZERO
      ...
      IF (JOK .EQ. -1 .AND. MITER .EQ. 1) THEN
C If JOK = -1 and MITER = 6, call JAC to evaluate Jacobian in sparse
c     form
      NJE = NJE + 1
      NSLJ = NST
      JCUR = 1
...
C
      ...
      DOUBLE PRECISION DI, HRL1, PHRL1, R
C-----------------------------------------------------------------------
C The following Fortran-77 declaration is to cause the values of the
C listed (local) variables to be saved between calls to this integrator.
C-----------------------------------------------------------------------
...
C
      integer, parameter :: lenrv1 = 48, leniv1 = 33, lenrv2 = 1, 
     &                      leniv2 = 8
...
C**End
      LOGICAL ISET
      INTEGER IPAR, IVALUE, IUMACH
C-----------------------------------------------------------------------
c      INTEGER IUMACH, LUNIT, MESFLG
C-----------------------------------------------------------------------
C The following Fortran-77 declaration is to cause the values of the
C listed (local) variables to be saved between calls to this routine.
C-----------------------------------------------------------------------
      integer :: lunit = -1, mesflg = 1
!$OMP THREADPRIVATE(lunit,mesflg)
c      SAVE LUNIT, MESFLG
c      DATA LUNIT/-1/, MESFLG/1/
