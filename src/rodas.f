...
C ----------------------------------------------------------
     ...
     *  FJAC(LDJAC,N),E(LDE,NM1),FMAS(LDMAS,NM1),ATOL(*),RTOL(*)
      ...
      COMMON/LINAL2/MLE,MUE,MBJAC,MBB,MDIAG,MDIFF,MBDIAG
      COMMON /CONROS/XOLD,HOUT,NN
!$OMP THREADPRIVATE(/LINAL2/,/CONROS/)
      type(sparse) :: sparseJ
C *** *** *** *** *** *** ***
C  INITIALISATIONS
C *** *** *** *** *** *** ***    
...
C --- COMPUTE JACOBIAN MATRIX ANALYTICALLY
         ...
         !call constV_jac_RADAUS(N,X,Y,sparseJ)
      END IF
      IF (.NOT.AUTNMS) THEN
         IF (IDFX.EQ.0) THEN
...
c     &            M1,M2,NM1,FAC,E,LDE,IP,IER,IJOB,IMPLCT,IP)

c     Federico Perini: substituted with sparse linear algebra
      R5_sys1 = identity(n, real(fac, dp)) - JAC_sparse
!      call matrix_details(R5_sys1,'R5_sys1')
!      call matrix_details(sparseJ,'sparseJ')
