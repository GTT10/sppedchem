! **********************************************************************
!     Version modified by Federico Perini to accomplish for sparse
!     Jacobian matrix provided in analytical form (MF = 26)
!     for stiff systems of ODEs.
!     Federico Perini, 03/06/2012
! **********************************************************************
...
!
   ...
   &RH, RTOLI, SIZE, TCRIT, TNEXT, TOLSF, TP
   INTEGER I, IER, IFLAG, IMXER, JCO, KGO, LENIW, LENJ, LENP, LENRW,&
   &LENWM, LF0, MBAND, MFA, ML, MU, NITER,&
   &NSLAST
   CHARACTER*80 MSG
!
...
! **** [fp] now option 26 (stiff problem, analytical sparse JAC) is
!      allowed
...
!
   ...
   &EM0, FLOTI, FLOTL, FLOTNQ, HSUM, RXI, RXIS, S,&
   &T1, T2, T3, T4, T5, T6, XI
...
!        YH(1:n, j) = EL(j)*YH(1:n, LP1) + YH(1:n, j)
370 CONTINUE
...
110   WM(I+2) = ZERO
   ...
   IF (JOK .EQ. -1 .AND. MITER .EQ. 1) THEN
! If JOK = -1 and MITER = 6, call JAC to evaluate Jacobian in sparse
!     form
      NJE = NJE + 1
      NSLJ = NST
      JCUR = 1
