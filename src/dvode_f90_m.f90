...
!$OMP THREADPRIVATE (USE_JACSP,LIKE_ORIGINAL_VODE,INFODS,LIWADS,MAXGRPDS,&
!$OMP MINGRPDS,NRFJACDS,NCFJACDS,LWKDS,LIWKDS,INDROWDS,INDCOLDS,NGRPDS,&
!$OMP IPNTRDS,JPNTRDS,IWADS,IWKDS,IOPTDS,YSCALEDS,WKDS,FACDS,U125,U325)
!_______________________________________________________________________
...
!$OMP   CONSECUTIVE_EFAILS, ELBOW_ROOM, IADIM, IANPIV, IAVPIV,                  &
!$OMP   ICF, ICNCP, IFAIL, IMAX, IMIN, INEWJ, INIT, IPUP, IRANK, IRFND, IRNCP,  &
!$OMP   ISTART, ISTATC, ITASKC, JADIM, JCUR, JMIN, JSTART, JSV, KFLAG, KOUNTL,  &
!$OMP   KUTH, L, LARGE, LAST, LENIGP, LICN_ALL, LIRN_ALL, LIW, LIWM, LMAX,      &
!$OMP   LOCJS, LP, LRW, LWM, LWMDIM, LWMTEMP, LYH, LYHTEMP, MANPIV, MAPIV, MAXG,&
!$OMP   MAXIT, MAXORD, MB28, MB48, METH, MICN, MICNCP, MINICN, MINIRN, MIRANK,  &
!$OMP   MIRN, MIRNCP, MITER, MLP, MOSS, MP, MSBG, MSBJ, MXHNIL, MXSTEP, N, NZB, &
!$OMP   NCFN, NDROP, NDROP1, NDX, NETF, NEWH, NEWQ, NFE, NGC, NGE, NGP, NHNIL,  &
!$OMP   NJE, NLP, NLU, NNI, NNZ, NOITER, NQ, NQNYH, NQU, NQWAIT, NSLG, NSLJ,    &
!$OMP   NSLP, NSRCH, NSRCH1, NST, NSUBS, NSUPS, NUM, NUMNZ, NYH, NZ_ALL,        &
!$OMP   NZ_SWAG, PREVIOUS_MAXORD, WPD, WPS, MA28AD_CALLS, MA28BD_CALLS,         &
!$OMP   MA28CD_CALLS, MC19AD_CALLS, MAX_MINIRN, MAX_MINICN, MAX_NNZ, BNGRP)
...
! *****MA48 build change point. Insert the above line.
      LOGICAL, PRIVATE :: ABORT, ABORT1, ABORT2, ABORT3, ABORTA, ABORTB,        &
        ...
        OK_TO_CALL_MA28, SUBS, SUPS, REDO_PIVOT_SEQUENCE,                       &
        SCALE_MATRIX, SPARSE, USE_FAST_FACTOR, YMAXWARN  
      LOGICAL, PUBLIC :: OPTS_CALLED
!$OMP THREADPRIVATE( ABORT, ABORT1, ABORT2, ABORT3, ABORTA, ABORTB,        &
!$OMP   ALLOW_DEFAULT_TOLS, BUILD_IAJA, BOUNDS, CHANGED_ACOR, GROW, IAJA_CALLED,&
!$OMP   J_HAS_BEEN_COMPUTED, J_IS_CONSTANT, LBIG, LBIG1, LBLOCK, MA48_WAS_USED, &
!$OMP   OK_TO_CALL_MA28, SUBS, SUPS, OPTS_CALLED, REDO_PIVOT_SEQUENCE,          &
!$OMP   SCALE_MATRIX, SPARSE, USE_FAST_FACTOR, YMAXWARN)
! ..
! .. Local Arrays ..
      REAL (WP), ALLOCATABLE, PRIVATE :: ACOR(:), CSCALEX(:), EWT(:),           &
        ...
        YTEMP(:), DTEMP(:)
!$OMP THREADPRIVATE(ACOR,CSCALEX,EWT,FPTEMP,FTEMP,FTEMP1,G0,G1,GX,JMAT,LB,PMAT,&
!$OMP RSCALEX,RWORK,SAVF,UB,WM,WMTEMP,WSCALEX,YHNQP2,YHTEMP,YMAX,YNNEG,YTEMP,&
!$OMP DTEMP)
...
!$OMP JAB,JAN,JATEMP,JGP,JROOT,JVECT,SUBDS,SUPDS)
