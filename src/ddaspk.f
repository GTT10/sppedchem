...
C      systems involved (INFO(12) = 0), or sparse systems(INFO(12)=2)
...
C           (or perhaps a dummy routine to satisf1y the loader).
...
C     LENIW and LENRW, using direct methods (INFO(12) = 0, 2) or
...
C
      ...
      LSAVR = LDELTA
      IF (INFO(12) == 1 0) LSAVR = LDELTA + NEQ
      LE = LSAVR + NEQ
      LWT = LE + NEQ
      LVT = LWT
...
C
C     Call the SPARSE nonlinear Newton solver for up to MXNIT iterations.
C
...
C
C     Compute a new step vector DELTA by back-substitution.
C     (updates DASPK_sys matrix of type(sparse_ordered)
...
C
C     Do Sparse-matrix LU decomposition on jacmat.
C
      DASPK_sys = jacmat
