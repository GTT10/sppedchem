...
C
     ...
     &            M1,M2,NM1,FAC1,E1,LDE1,IP1,IER,IJOB,CALHES,IPHES)
      ...
      use sparse_algebra, only: sparse, dense_to_sparse
...
c      write(*,*)'B=IDENTITY, JACOBIAN A FULL MATRIX'
      DO J=1,N
         DO  I=1,N
            E1(I,J)=-FJAC(I,J)
         END DO
...
!$OMP THREADPRIVATE(/LINAL/)
C
      double precision, dimension(n) :: solved_sparse
      integer, dimension(n) :: row_ord, col_ord, inv_col_ord
      integer                 :: path, lwork, liwork, iexcess, err_flag
...
C
     ...
     &     SCAL(N),IPHES(N),Z1(N),Z2(N),Z3(N),F1(N),F2(N),Y0(N),Y(N)
      ...
      double precision, dimension(n) :: unk1
...
C ------  B=IDENTITY, JACOBIAN A FULL MATRIX
      ...
      unk1 = cont(1:n)
      cont(1:n) = R5_sys1.backslash.unk1
...
c         res1 = sparse_linear_system(msys1,unk1,iper1,iiper1,iLU1,rLU1)
c         cont(1:n) = res1
         cont(1:n) = R5_sys1.backslash.unk1
c          CALL SOL(N,LDE1,E1,CONT,IP1)
          GOTO 88
...
C
     ...
     &          IP(NM1),DY(N),AK(N),FX(N),YNEW(N),tmp(n)
      LOGICAL STAGE1
      COMMON/LINAL/MLE,MUE,MBJAC,MBB,MDIAG,MDIFF,MBDIAG
!$OMP THREADPRIVATE(/LINAL/)
C
...
c      CALL SOL (N,LDE,E,AK,IP)
      tmp(1:n) = AK(1:n)
      AK(1:N) = R5_sys1 .backslash. tmp(1:N)
